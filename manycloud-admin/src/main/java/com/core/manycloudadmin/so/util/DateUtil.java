package com.core.manycloudadmin.so.util;

import com.core.manycloudcommon.utils.DateStyle;
...
//        Week week = null;
     ...
     * 对日期进行月份偏移
     * @param date 要偏移的日期
     * @param amount 偏移的月份数（正数为向后偏移，负数为向前偏移）
     * @return 偏移后的日期
     */
     ...
     * 对日期进行偏移（年、月、日维度）
     * @param date 要偏移的日期
     * @param dateStyle 时间维度（Y:年,M:月,D:日）
     * @param amount 偏移量
     * @return 偏移后的日期
     */
    public static Date offset(Date date, String dateStyle, int amount) {
        if (date == null || dateStyle == null) {
            return null;
        }
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        if ("Y".equals(dateStyle)) {
            calendar.add(Calendar.YEAR, amount);
        } else if ("M".equals(dateStyle)) {
            calendar.add(Calendar.MONTH, amount);
        } else if ("D".equals(dateStyle)) {
     ...
     * @param endDate 结束日期
     * @param timeUnit 时间维度（Y:年,M:月,D:日）
     * @param formatStr 日期格式字符串
     * @return 时间区间字符串列表
     */
    public static List<String> rangeToList(Date startDate, Date endDate, String timeUnit, String formatStr) {
        List<String> dateList = new ArrayList<>();
        if (startDate == null || endDate == null || formatStr == null) {
            return dateList;
        }
